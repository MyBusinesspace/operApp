...
function EntryRow({ entry, period, onApprove, approving, onEdit, empMap }) {
...
export default function PayrollEntriesModal({ period, onClose, onRefresh }) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          {entries.length > 0 && (
            <Button variant="outline" size="sm" className="gap-1.5 shrink-0" onClick={handlePrintAll}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {sortedEntries.map(entry => (
                  <EntryRow key={entry.id} entry={entry} period={period} onApprove={handleApproveEntry} approving={approving} onEdit={handleEntrySaved} empMap={empMap} />
                ))}
              </tbody>
            </table>
          )}
