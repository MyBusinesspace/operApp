...
export default function ProjectFiles({ projectId, files, onRefresh }) {
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.ProjectFile.delete(id);
    onRefresh();
  };
            ...
            <tbody>
              {[...localFiles].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)).map((f, i) => (
                <tr key={f.id || i} className="border-b border-border/50 hover:bg-muted/20 transition-colors group">
                  ...
                  <td className="px-4 py-2 hidden sm:table-cell">
                    <FileTypeCell file={f} entityName="ProjectFile" onUpdated={handleTypeUpdated} />
                  </td>
