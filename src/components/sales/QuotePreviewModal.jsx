...
export function DocContent({ quote, contact, template: t, accentColor, currency, title, page = FULL_PAGE, incotermExplanation = "", measure = false }) {
  ...
  const billName = contact?.fiscal_legal_name || contact?.full_name || contact?.company || quote.contact_name || "—";
  ...
  const lineItems = (quote.line_items || []).filter(l => (l.description || "").trim() !== "" || (Number(l.total) || 0) > 0);
    ...
    <div data-doc-root style={{ fontFamily: t.font || "Inter", fontSize: 9.5, color: "#1a1a2e", padding: "24px 30px 64px", boxSizing: "border-box", height: measure ? "auto" : "100%", position: "relative", display: "flex", flexDirection: "column" }}>
      ...
      {!isContinuation && (
        <div data-block="header">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 8.5, color: "#666", marginBottom: 10 }}>
                {[
                  ...
                  quote.issue_date && `Date: ${fmtDate(quote.issue_date)}`,
                  quote.expiry_date && `Expiry: ${fmtDate(quote.expiry_date)}`,
              ...
              {(() => {
                const _taskRefs = (quote.task_references?.length ? quote.task_references : quote.task_names || []).filter(Boolean);
                const rel = [
                  quote.reference ? `Ref: ${quote.reference}` : "",
                  quote.project_name ? `Project: ${quote.project_name}` : "",
                  quote.work_order_name ? `WO: ${quote.work_order_name}` : "",
          ...
          {/* Doc title & summary */}
          {(quote.title || quote.doc_summary) && (
            <div style={{ marginBottom: 12 }}>
              {quote.title && <div style={{ ...headFill, fontSize: 12 }}>{quote.title}</div>}
              {quote.doc_summary && <div style={{ ...bodyBox, fontSize: 8.5, color: "#555", lineHeight: 1.6 }}>{quote.doc_summary}</div>}
            </div>
          )}
        </div>
      )}
      ...
      {isContinuation && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, paddingBottom: 6, borderBottom: `1px solid #ddd` }}>
          <div style={{ fontSize: 9, color: "#888" }}>{title} {quote.number ? `#${quote.number}` : ""} — continued</div>
        ...
        <div data-block="totals" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 12, gap: 16 }}>
          <div style={{ fontSize: 8, color: "#555", fontStyle: "italic", maxWidth: 340, paddingBottom: 4, lineHeight: 1.4 }}>
            {Number(quote.total || 0) > 0 && (
              <span><strong style={{ fontStyle: "normal" }}>Amount in words:</strong> {amountToWords(quote.total, currency)}</span>
            )}
            {quote.incoterm && incotermExplanation && (
              <div style={{ marginTop: 4, fontStyle: "normal", fontSize: 7.5, color: "#777", lineHeight: 1.4 }}>
                <strong style={{ fontWeight: 700 }}>{quote.incoterm}</strong> — {incotermExplanation}
              </div>
            )}
          </div>
          <div style={{ minWidth: 220 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, fontWeight: 700, paddingTop: 4 }}>
              <span>Total {quote.incoterm || "(including Tax)"} {currency}</span><span>{Number(quote.total || 0).toFixed(2)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Notes */}
      {quote.notes && showNotes && (
        <div data-block="notes" style={{ marginBottom: 12 }}>
          <div style={{ ...headFill, fontSize: bodyFs + 0.5 }}>Notes</div>
          <div style={{ ...bodyBox, fontSize: bodyFs, color: "#444", whiteSpace: "pre-line", lineHeight: 1.6 }}>{quote.notes}</div>
        </div>
      )}

      {/* Terms & Conditions (shown on document) */}
      {quote.terms && quote.terms.trim() && showTerms && (
        <div data-block="terms" style={{ marginBottom: 12 }}>
          <div style={{ ...headFill, fontSize: bodyFs + 0.5 }}>Terms &amp; Conditions</div>
          <div style={{ ...bodyBox, fontSize: bodyFs, color: "#444", whiteSpace: "pre-line", lineHeight: 1.6 }}>{quote.terms}</div>
        </div>
      )}
...
export function calcPageBreak(quote, template) {
  ...
  const tailRows = textRowUnits(quote.notes) + textRowUnits(quote.terms) + 2;
...
export default function QuotePreviewModal({ open, onClose, quote, contact, template, onPrint, onEdit }) {
  ...
  // Derived values (defensive against null quote/template) — needed by the
  ...
  useLayoutEffect(() => {
    ...
    const renderMeasure = async (fs) => {
      const html = renderToString(React.createElement(DocContent, {
        quote, contact, template: { ...t, body_font_size: fs }, accentColor, currency, title, page: FULL_PAGE, incotermExplanation, measure: true,
      }));
  ...
  }, [open, quote, template, accentColor, currency, title, incotermExplanation]);
  ...
  }, [open, quote, template, accentColor, currency, title, incotermExplanation, autoFs]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border shrink-0">
          <span className="font-semibold text-sm">Quote Preview — {quote.number || "Draft"}</span>
        ...
        <div className="overflow-y-auto flex-1 bg-slate-200 dark:bg-slate-800 p-6 flex flex-col items-center gap-6">
          {pages.map((page, idx) => (
            <div key={idx} className="relative shrink-0" style={wrapperStyle}>
              <div className="bg-white shadow-xl rounded-sm overflow-hidden" style={{ width: A4_W, height: A4_H, transform: `scale(${SCALE})`, transformOrigin: "top left" }}>
                <DocContent quote={quote} contact={contact} template={fitT} accentColor={accentColor} currency={currency} title={title} page={page} incotermExplanation={incotermExplanation} />
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-300 p-5">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2 mb-3">
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400">{quote.number || ""}</div>
                  </div>
          ...
          {/* Annex quick view — inline right below the annex grid, at the bottom of the page */}
        ...
        <div aria-hidden style={{ position: "fixed", left: "-10000px", top: 0, width: A4_W, pointerEvents: "none", opacity: 0 }}>
          <div ref={measureRef} style={{ width: A4_W }}>
            <DocContent quote={quote} contact={contact} template={fitT} accentColor={accentColor} currency={currency} title={title} page={FULL_PAGE} incotermExplanation={incotermExplanation} measure />
          </div>
        </div>
      </div>
    </div>
