...
export default function ContactFiles({ contactId, files, onRefresh }) {
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.ContactFile.delete(id);
    onRefresh();
  };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <SharedFileUploadDialog
          entityName="ContactFile"
          parentIdField="contact_id"
          parentIdValue={contactId}
          onDone={onRefresh}
          trigger={
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-foreground leading-tight truncate">{f.file_name}</p>
                        {f.description && <p className="text-xs text-muted-foreground">{f.description}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2 hidden sm:table-cell">
                    <FileTypeCell file={f} entityName="ContactFile" onUpdated={handleTypeUpdated} />
                  </td>
