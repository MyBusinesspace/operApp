...
export default function WorkOrderFiles({ workOrderId, files, onRefresh }) {
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.WorkOrderFile.delete(id);
    onRefresh();
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <SharedFileUploadDialog
          entityName="WorkOrderFile"
          parentIdField="work_order_id"
          parentIdValue={workOrderId}
          onDone={onRefresh}
          trigger={
            ...
            <tbody>
              {[...localFiles].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)).map((f, i) => (
                <tr key={f.id || i} className="border-b border-border/50 hover:bg-muted/20 transition-colors group">
                  ...
                  <td className="px-4 py-2 hidden sm:table-cell">
                    <FileTypeCell file={f} entityName="WorkOrderFile" onUpdated={handleTypeUpdated} />
                  </td>
