...
const ACTION_ICONS = {
  Note: MessageSquare, Call: Phone, Email: Mail, Meeting: Users, Update: Calendar,
};
const ACTION_COLORS = {
  Note: "bg-slate-100 text-slate-600", Call: "bg-blue-100 text-blue-600",
  Email: "bg-violet-100 text-violet-600", Meeting: "bg-emerald-100 text-emerald-600",
...
export default function WorkOrderHistory({ workOrderId, notes, onRefresh }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editing) await base44.entities.WorkOrderNote.update(editing.id, form);
    else await base44.entities.WorkOrderNote.create({ work_order_id: workOrderId, ...form });
    setSaving(false);
    setShowModal(false);
    onRefresh();
  };
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.WorkOrderNote.delete(id);
    onRefresh();
  };
  ...
  return (
    <div>
      ...
      {notes.length === 0 ? (
        <div className="py-10 text-center text-sm text-muted-foreground">No history yet. Add the first note.</div>
      ) : (
              ...
              {[...notes].sort((a, b) => new Date(b.created_date) - new Date(a.created_date)).map((n, i) => {
                ...
                return (
                  <tr key={n.id || i} className="border-b border-border/50 hover:bg-muted/20 transition-colors group">
                    <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{fmtDatetime(n.created_date)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${ACTION_COLORS[n.action] || ACTION_COLORS.Note}`}>
                        <ActionIcon className="w-3 h-3" />{n.action || "Note"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-foreground">{n.note}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground" onClick={() => openEdit(n)}>
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1 rounded hover:bg-muted text-muted-foreground hover:text-destructive" onClick={() => handleDelete(n.id)}>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 pt-1">
            <Select value={form.action} onValueChange={v => setForm(f => ({ ...f, action: v }))}>
              ...
              <SelectContent>
                {["Note","Call","Email","Meeting","Update"].map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}
              </SelectContent>
            </Select>
            <Textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="Write your note here..." rows={4} />
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowModal(false)}>Cancel</Button>
              <Button onClick={handleSave} disabled={saving || !form.note.trim()}>{saving ? "Saving..." : editing ? "Update" : "Add Note"}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
