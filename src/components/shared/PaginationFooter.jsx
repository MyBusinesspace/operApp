...
/**
 * Xero-style pagination footer — drop-in replacement for DataTablePagination.
 * Uses the same `pagination` object returned by useTablePagination().
 *
 * Left: page selector + items-per-page selector + total count.
 * Right: page number links with Start / Prev / Next / End controls.
 */
export default function PaginationFooter({ pagination, pageSizeOptions = [100, 200, 500] }) {
  const { page, setPage, pageSize, setPageSize, totalItems, totalPages } = pagination;
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 py-2.5 bg-muted/30 border border-border rounded-xl text-xs text-muted-foreground mt-3">
