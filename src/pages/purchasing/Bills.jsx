...
import {
  ...
  AlertCircle, CheckCircle2, Send, Ban, RefreshCw,
  MoreVertical, Copy, ChevronRight, SlidersHorizontal, X, Eye, Printer
} from "lucide-react";
...
export default function Bills() {
  ...
  useEffect(() => {
    ...
    if (statusParam) {
      ...
      navigate("/purchasing/bills", { replace: true });
    } else if (params.get("create") === "true") {
      navigate("/purchasing/bills/new", { replace: true });
    }
  }, [location.search]);

  const countBy = (s) => bills.filter(b => b.status === s).length;
  ...
  const bulkStatus = async (newStatus) => {
    await Promise.all([...selected].map(id => base44.entities.Bill.update(id, { status: newStatus })));
    setSelected(new Set()); load();
  };

  const bulkDelete = async () => {
    if (!confirm(`Delete ${selected.size} bill(s)?`)) return;
    await Promise.all([...selected].map(id => base44.entities.Bill.delete(id)));
    setSelected(new Set()); load();
  };

  const handleDuplicate = (b) => {
    const { id, created_date, updated_date, created_by_id, number, ...rest } = b;
    navigate("/purchasing/bills/new", { state: { duplicate: { ...rest, status: "Draft", number: "" } } });
  };

  const openEdit = (b) => navigate(`/purchasing/bills/${b.id}/edit`);
  const openAdd  = () => navigate("/purchasing/bills/new");

  const selectedTotal = filtered.filter(b => selected.has(b.id)).reduce((s, b) => s + (b.total || 0), 0);
  const firstCurrency = filtered.find(b => selected.has(b.id))?.currency || "AED";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-card border-b border-border px-6 py-3 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Enter a supplier, amount, reference or bill number"
      ...
      <div className="bg-card border-b border-border px-6 py-2 flex items-center justify-between min-h-[40px]">
        <span className="text-xs text-muted-foreground">
          ...
          {filterStatus !== "all" && ` | ${fmtAmount(filtered.reduce((s, b) => s + (b.total || 0), 0), filtered[0]?.currency || "AED")}`}
          {selected.size > 0 && <span className="ml-3 font-medium text-foreground">{selected.size} selected · {fmtAmount(selectedTotal, firstCurrency)}</span>}
        </span>
            ...
            <>
              ...
              {filterStatus === "Draft" && (
                <>
                  ...
                  <Button size="sm" className="h-7 text-xs px-3 bg-slate-800 hover:bg-slate-900 text-white" disabled={selected.size === 0} onClick={() => bulkStatus("Awaiting Payment")}>Approve</Button>
                </>
              )}
              ...
              {filterStatus === "Awaiting Payment" && (
                <Button size="sm" className="h-7 text-xs px-3 bg-emerald-600 hover:bg-emerald-700 text-white" disabled={selected.size === 0} onClick={() => bulkStatus("Paid")}>Mark as Paid</Button>
              )}
            </>
          )}
        </div>
      ...
      <div className="flex-1 overflow-auto bg-card">
        {loading ? (
          <div className="p-8 text-center text-muted-foreground text-sm border-t border-border">Loading bills...</div>
        ) : filtered.length === 0 ? (
          <div className="p-16 text-center border-t border-border">
            <div className="w-16 h-16 rounded-2xl bg-muted flex items-center justify-center mx-auto mb-4">
              <FileText className="w-7 h-7 text-muted-foreground/50" />
            </div>
            <h3 className="text-lg font-semibold text-foreground mb-1">No bills found</h3>
            <p className="text-sm text-muted-foreground mb-6">
              {search || filterStatus !== "all" ? "Try adjusting your filters." : "Create your first bill to get started."}
            </p>
                  ...
                  {pagination.pageItems.map(b => {
                    ...
                    return (
                      <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        onClick={() => openEdit(b)}>
                        {filterStatus !== "all" && (
                          <td className="px-4 py-3 w-10" onClick={e => { e.stopPropagation(); toggleSelect(b.id, e.shiftKey); }}>
                            <input type="checkbox" className="rounded border-border" checked={selected.has(b.id)}
                              onClick={e => { e.stopPropagation(); toggleSelect(b.id, e.shiftKey); }} readOnly />
                          </td>
                        )}
                        <td className="px-4 py-3"><span className="text-sm font-semibold text-primary hover:underline">{b.number || "—"}</span></td>
                        <td className="px-4 py-3 max-w-[160px]"><p className="text-sm text-foreground truncate">{b.reference || "—"}</p></td>
                        <td className="px-4 py-3 max-w-[200px]" onClick={e => e.stopPropagation()}>
                          {b.contact_id ? (
                            <Link to={`/contacts/${b.contact_id}`} className="text-sm font-medium text-primary hover:underline">{b.contact_name || "—"}</Link>
                          ) : (
                        ...
                        <td className="px-4 py-3 hidden md:table-cell">
                          <div className="flex flex-col gap-1">
                            <span className="flex items-center gap-1 text-xs text-muted-foreground"><Clock className="w-3 h-3 shrink-0" />{fmtDate(b.issue_date)}</span>
                            {b.due_date ? (
                              <span className={`flex items-center gap-1 text-xs ${overdue ? "text-red-600 font-medium" : "text-muted-foreground/60"}`}>
                                {overdue ? <AlertCircle className="w-3 h-3 shrink-0" /> : <span className="w-3 h-3 shrink-0" />}{fmtDate(b.due_date)}
                              </span>
                        ...
                        <td className="px-4 py-3 hidden lg:table-cell max-w-[160px]" onClick={e => e.stopPropagation()}>
                          <div className="flex flex-col gap-0.5">
                            {b.project_name && <span className="text-xs font-medium text-foreground truncate">{b.project_name}</span>}
                            {b.work_order_name && <span className="text-xs text-muted-foreground truncate">{b.work_order_name}</span>}
                            {!b.project_name && !b.work_order_name && <span className="text-xs text-muted-foreground/40">—</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}><StatusBadge status={b.status} /></td>
                        <td className="px-4 py-3 text-right"><span className="text-sm font-semibold tabular-nums">{fmtAmount(b.total, b.currency)}</span></td>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-44">
                                ...
                                <DropdownMenuItem onClick={() => handleDuplicate(b)}>
                                  <Copy className="w-4 h-4 mr-2" /> Duplicate Bill
                                </DropdownMenuItem>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setPreview(null)}>
          <div className="bg-card border border-border rounded-2xl shadow-2xl p-6 max-w-lg w-full mx-4 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-foreground">{preview.number || "Bill"}</h3>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><p className="text-xs text-muted-foreground">Due Date</p><p className={`font-medium ${isOverdue(preview) ? "text-red-600" : ""}`}>{fmtDate(preview.due_date)}</p></div>
            ...
            <div className="flex gap-2 justify-end pt-1">
              ...
              <Button onClick={() => { setPreview(null); openEdit(preview); }}>Edit Bill</Button>
            </div>
          </div>
        </div>
      )}
